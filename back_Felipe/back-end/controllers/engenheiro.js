const Engenheiro = require('../models/Engenheiro');

const controller = {} // Objeto vazio

controller.novo = async (req, res) => {
   try {
      await Engenheiro.create(req.body)
...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) {
      ...
      try {
         ...
         const lista = await Engenheiro.find()
...
controller.obterUm = async (req, res) => {
   try {
      ...
      const obj = await Engenheiro.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      const id = req.params.id
      const obj = await Engenheiro.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      ...
      const obj = await Engenheiro.findByIdAndDelete(id)
      if(obj) {
         res.status(204).end()
      }
      else {
...
async function busca (req, res) {
   ...
   try{
      let lista = await Engenheiro.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
