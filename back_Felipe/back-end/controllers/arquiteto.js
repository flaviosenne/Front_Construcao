const Arquiteto = require('../models/Arquiteto');

const controller = {} // Objeto vazio

controller.novo = async (req, res) => {
   try {
      await Arquiteto.create(req.body)
...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) {
      ...
      try {
         ...
         const lista = await Arquiteto.find()
...
controller.obterUm = async (req, res) => {
   try {
      ...
      const obj = await Arquiteto.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      const id = req.params.id
      const obj = await Arquiteto.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      ...
      const obj = await Arquiteto.findByIdAndDelete(id)
      if(obj) {
         res.status(204).end()
      }
      else {
...
async function busca (req, res) {
   ...
   try{
      let lista = await Arquiteto.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
