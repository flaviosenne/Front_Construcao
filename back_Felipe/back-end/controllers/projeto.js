...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) {
      ...
      try {
         ...
         const lista = await Projeto.find().populate('cliente').populate('arquiteto') 
...
controller.obterUm = async (req, res) => {
   try {
      ...
      const obj = await Projeto.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      const id = req.params.id
      const obj = await Projeto.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      ...
      const obj = await Projeto.findByIdAndDelete(id)
      if(obj) {
         res.status(204).end()
      }
      else {
...
async function busca (req, res) {
   ...
   try{
      let lista = await Projeto.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
